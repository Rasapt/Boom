...
def get_image_vector_store() -> VectorStore:
    """
    Separate Chroma collection for page-image embeddings.

    Kept apart from the text-chunk collection (different embedding
    dimensions/spaces) and namespaced by the active embedding backend, so
    switching IMAGE_EMBEDDING_BACKEND never mixes incompatible vectors
    together. For image chunks, `content` in similarity_search() results
    is the on-disk path to that page's image file, and `chunk_index` is
    the zero-based page number.
    """
    settings = get_settings()

    collection_name = (
        f"{settings.CHROMA_IMAGE_COLLECTION}_{settings.IMAGE_EMBEDDING_BACKEND.lower()}"
    )
