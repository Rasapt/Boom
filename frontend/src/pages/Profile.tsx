...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="bg-[#FAFAF8] rounded-xl px-4 py-3">
      <p className="text-[10px] font-bold text-[#A89888] mb-1.5 text-right">{label}</p>
      {children}
    </div>
  );
}
      ...
      {options.map(o => (
        <button key={o} onClick={() => onChange(o)}
          className={`py-1.5 px-3 rounded-xl text-[12px] font-semibold transition-all ${
            value === o ? "bg-[#C4714A] text-[#F8F6F2]" : "bg-[#F0EBE3] text-[#5A4030] hover:bg-[#E5DDD4]"
          }`}
        >{o}</button>
      ))}
    </div>
...
export default function Profile({ nav, userData }: { nav: NavFn; userData: SignupData | null }) {
  ...
  const sections = [
    ...
    <div className="space-y-3" key="general">
      <div className="grid grid-cols-2 gap-2">
        <Field label="نام">
          <input value={firstName} onChange={e => setFirstName(e.target.value)} placeholder="پریسا"
            className="w-full bg-transparent outline-none text-[14px] font-semibold text-[#1A1108] placeholder:text-[#C4B8A8] text-right" />
        </Field>
      ...
      <Field label="شماره موبایل">
        <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="۰۹۱۲۳۴۵۶۷۸۹" dir="ltr"
          className="w-full bg-transparent outline-none text-[14px] font-semibold text-[#1A1108] placeholder:text-[#C4B8A8] text-left" />
      </Field>
      <Field label="تاریخ تولد">
        <input type="date" value={birthday} onChange={e => setBirthday(e.target.value)} dir="ltr"
          className="w-full bg-transparent outline-none text-[14px] font-semibold text-[#1A1108] text-left" />
      </Field>
      <Field label="شهر">
    ...
    <div className="space-y-3" key="lifestyle">
      <Field label="معمولاً چه ساعتی بیدار می‌شی؟">
        <input type="time" value={wakeTime} onChange={e => setWakeTime(e.target.value)} dir="ltr"
          className="w-full bg-transparent outline-none text-[14px] font-semibold text-[#1A1108]" />
      </Field>

      <div className="bg-[#FAFAF8] rounded-xl px-4 py-3">
        <p className="text-[10px] font-bold text-[#A89888] mb-3 text-right">ساعت مطالعه در هر روز هفته</p>
        <div className="space-y-3">
          {DAYS_FA.map(d => (
            <div key={d} className="flex items-center gap-3">
              <span className="text-[12px] font-bold text-[#C4714A] w-8 text-left flex-shrink-0">{dailyHours[d]}س</span>
              <input type="range" min={0} max={12} step={0.5} value={dailyHours[d]}
                onChange={e => setDailyHours(h => ({ ...h, [d]: Number(e.target.value) }))}
                className="flex-1" />
              <span className="text-[12px] font-bold text-[#7A6858] w-14 text-right flex-shrink-0">{d}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-[#FAFAF8] rounded-xl px-4 py-3">
        <p className="text-[10px] font-bold text-[#A89888] mb-2 text-right">حداکثر ساعت مطالعه‌ی پشت سر هم</p>
        <div className="flex items-center gap-3">
          <span className="text-[13px] font-bold text-[#C4714A] flex-shrink-0">{maxConsec} ساعت</span>
      ...
      <div className="bg-[#FAFAF8] rounded-xl px-4 py-3">
        <p className="text-[10px] font-bold text-[#A89888] mb-2 text-right">ساعت خواب شبانه</p>
        <div className="flex items-center gap-3">
          <span className="text-[13px] font-bold text-[#C4714A] flex-shrink-0">{sleepHours} ساعت</span>
    ...
    <div className="space-y-3" key="subjects">
      <p className="text-[12px] text-[#7A6858] leading-relaxed text-right">
      ...
      {subjects.map(s => (
        <div key={s} className="bg-[#FAFAF8] rounded-2xl p-4">
          <p className="text-[13px] font-bold text-[#1A1108] mb-3 text-right">{s}</p>
          <div className="space-y-3">
            <div>
              <div className="flex justify-between mb-1">
                <span className="text-[11px] font-bold text-[#C4714A]">{completion[s]}%</span>
                <span className="text-[11px] font-bold text-[#A89888]">میزان پیشرفت در کتاب</span>
              </div>
            ...
            <div>
              <div className="flex justify-between mb-1">
                ...
                <span className="text-[11px] font-bold text-[#A89888]">سطح اعتماد به نفس</span>
              </div>
    ...
    <div className="space-y-3" key="ai">
      ...
      <div className="bg-[#FAFAF8] rounded-xl px-4 py-3 flex items-center justify-between">
        <button
          ...
          className={`w-11 h-6 rounded-full transition-all relative flex-shrink-0 ${notifs ? "bg-[#C4714A]" : "bg-[#D5CCC3]"}`}
        >
        ...
        <div className="text-right">
          <p className="text-[13px] font-semibold text-[#1A1108]">اعلان‌های هوشمند</p>
          <p className="text-[11px] text-[#A89888] font-medium mt-0.5">یادآوری‌ها و آپدیت برنامه</p>
        </div>
      </div>
    </div>,
  ];
  ...
  return (
    <div className="min-h-screen bg-[#F8F6F2] pb-10">
      <div className="px-5 pt-12 pb-4 flex items-center gap-4 sticky top-0 bg-[#F8F6F2]/95 backdrop-blur-sm z-10 border-b border-[#F0EBE3]">
        <BackButton onClick={() => nav("home")} />
        <div className="text-right">
          <h1 className="font-display text-xl text-[#1A1108]">تکمیل پروفایل</h1>
          <p className="text-[12px] text-[#A89888] font-medium">به هوش مصنوعی کمک می‌کنه همه چیز رو شخصی کنه</p>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex overflow-x-auto px-5 py-3 gap-2 border-b border-[#F0EBE3]" style={{ scrollbarWidth: "none" }}>
        {SECTIONS.map((s, i) => (
          <button key={s} onClick={() => setActiveSection(i)}
            className={`flex-shrink-0 py-2 px-4 rounded-xl text-[12px] font-bold transition-all ${
              activeSection === i ? "bg-[#1A1108] text-[#F8F6F2]" : "bg-[#F0EBE3] text-[#7A6858] hover:bg-[#E5DDD4]"
            }`}
          >{s}</button>
        ))}
      </div>
      ...
      <div className="px-5">
        <button onClick={save}
          className={`w-full py-4 rounded-2xl font-bold text-[14px] transition-all active:scale-95 ${
            saved ? "bg-[#6B9E7A] text-white" : "bg-[#C4714A] text-[#F8F6F2] hover:bg-[#A85C38]"
          }`}
        >
