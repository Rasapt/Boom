          ...
          {MAJORS.map(m => (
            <button key={m} onClick={() => pick(setMajor, m)}
              className={`py-3.5 px-5 rounded-2xl text-right font-semibold text-[14px] transition-all ${
                major === m ? "bg-[#C4714A] text-[#F8F6F2]" : "bg-white border border-[#E5DDD4] text-[#3A2A1A] hover:border-[#C4714A]"
              }`}
            >{m}</button>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-2">
          {GRADES.map(g => (
            <button key={g} onClick={() => pick(setGrade, g)}
              className={`py-3.5 px-5 rounded-2xl text-right font-semibold text-[14px] transition-all ${
                grade === g ? "bg-[#C4714A] text-[#F8F6F2]" : "bg-white border border-[#E5DDD4] text-[#3A2A1A] hover:border-[#C4714A]"
              }`}
            >{g}</button>
          ))}
        </div>
          ...
          {EXAM_YEARS.map(y => (
            <button key={y} onClick={() => pick(setExamYear, y)}
              className={`py-4 rounded-2xl font-bold text-xl text-center transition-all ${
                examYear === y ? "bg-[#C4714A] text-[#F8F6F2]" : "bg-white border border-[#E5DDD4] text-[#3A2A1A] hover:border-[#C4714A]"
              }`}
            >{y}</button>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-2">
          {TARGET_RANKS.map(r => (
            <button key={r} onClick={() => pick(setTargetRank, r)}
              className={`py-3.5 px-5 rounded-2xl text-right font-semibold text-[14px] transition-all ${
                targetRank === r ? "bg-[#C4714A] text-[#F8F6F2]" : "bg-white border border-[#E5DDD4] text-[#3A2A1A] hover:border-[#C4714A]"
              }`}
            >{r}</button>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-2">
          {STUDY_HOURS_OPTIONS.map(h => (
            <button key={h} onClick={() => pick(setStudyHours, h)}
              className={`py-3.5 px-5 rounded-2xl text-right font-semibold text-[14px] transition-all ${
                studyHours === h ? "bg-[#C4714A] text-[#F8F6F2]" : "bg-white border border-[#E5DDD4] text-[#3A2A1A] hover:border-[#C4714A]"
              }`}
            >{h}</button>
          ))}
        </div>
          ...
          {TEST_EXAM_OPTIONS.map(e => (
            <button key={e} onClick={() => toggleExam(e)}
              className={`py-3 px-5 rounded-2xl text-right font-semibold text-[14px] transition-all flex items-center gap-3 ${
                testExams.includes(e) ? "bg-[#C4714A] text-[#F8F6F2]" : "bg-white border border-[#E5DDD4] text-[#3A2A1A] hover:border-[#C4714A]"
              }`}
            >
              <div className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center flex-shrink-0 ${
                testExams.includes(e) ? "border-white/50 bg-white/20" : "border-[#C4B8A8]"
              }`}>
