  ...
  return (
    <div className="min-h-screen flex flex-col bg-[var(--surface)] px-6 pt-14 pb-10">
      ...
      <div className="mt-10">
        ...
        <h1 className="font-display text-4xl text-[var(--text)] leading-snug">
          شماره موبایلت<br />چنده؟
        </h1>
        <p className="text-[13px] text-[var(--muted)] mt-2">یه کد تأیید برات می‌فرستیم.</p>
      </div>

      <div className="mt-10">
        {/* Phone input: LTR internally so +98 appears on left, digits flow left-to-right */}
        <div className="flex items-center bg-white border-2 border-[var(--border-strong)] focus-within:border-[var(--accent)] rounded-2xl overflow-hidden transition-colors" dir="ltr">
        ...
        <p className="text-[12px] text-[var(--muted-2)] mt-2">شماره موبایل ایرانی بدون صفر اول</p>
      </div>
      ...
      <p className="mt-5 text-center text-[13px] text-[var(--muted)]">
        هنوز ثبت‌نام نکردی؟{" "}
        <button onClick={() => nav("signup")} className="text-[var(--accent)] font-bold underline underline-offset-2">
          ثبت‌نام
        </button>
      </p>
    </div>
...
function OTPStep({ phone, onBack, onVerify }: {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[var(--surface)] px-6 pt-14 pb-10">
      ...
      <div className="mt-10">
        <p className="text-xs font-bold tracking-[0.2em] text-[var(--accent)] mb-1">تأیید هویت</p>
        <h1 className="font-display text-4xl text-[var(--text)] leading-snug">کد را وارد کن</h1>
        <p className="text-[13px] text-[var(--muted)] mt-2">
          ارسال شده به{" "}
          <span className="font-bold text-[var(--text)]" dir="ltr">{formattedPhone}</span>
        </p>
      ...
      <div className="mt-10 flex gap-2.5 justify-center" dir="ltr">
        {digits.map((d, i) => (
          <input
            ...
            className={`w-11 h-14 text-center text-xl font-bold rounded-2xl border-2 outline-none transition-all bg-white text-[var(--text)] ${
              d ? "border-[var(--accent)] bg-[var(--accent-soft)]" : "border-[var(--border-strong)] focus:border-[var(--accent)]"
            }`}
          />
        ))}
      </div>
      ...
      <div className="mt-6 text-center">
        {timer > 0 ? (
          <p className="text-[13px] text-[var(--muted)]">
            ارسال مجدد کد در{" "}
            <span className="font-bold text-[var(--accent)]" dir="ltr">0:{timer.toString().padStart(2, "0")}</span>
          </p>
        ) : (
          <button onClick={() => { setTimer(59); setDigits(["","","","","",""]); refs.current[0]?.focus(); }}
            className="text-[13px] font-bold text-[var(--accent)] underline underline-offset-2">
            ارسال مجدد کد
          </button>
        )}
      </div>
