  ...
  return (
    <div className="min-h-screen bg-[var(--surface)]">
      <div className="px-5 pt-12 pb-4 flex items-center gap-4">
        ...
        <h1 className="font-display text-xl text-[var(--text)]">استریک شما</h1>
      </div>

      {/* Streak hero */}
      <div className="mx-5 mt-2 bg-white rounded-3xl p-6 border border-[var(--border)] flex items-center gap-5">
        <div className="w-20 h-20 rounded-2xl bg-[var(--accent-soft)] flex items-center justify-center">
        ...
        <div>
          ...
          <p className="text-5xl font-bold text-[var(--accent)] leading-tight">۷</p>
      ...
      {/* Bar chart */}
      <div className="mx-5 mt-4 bg-white rounded-3xl p-5 border border-[var(--border)]">
          ...
          {PAST_RECORDS.map((r, i) => {
            ...
            return (
              <div key={r.date} className="flex flex-col items-center gap-1.5 flex-1">
                <span className="text-[9px] font-bold text-[var(--muted-2)]">{r.xp.toLocaleString("fa-IR")}</span>
                <div className="w-full flex items-end" style={{ height: "80px" }}>
                  <div className="w-full rounded-lg transition-all duration-700"
                    style={{
                      height: `${height}%`,
                      background: isToday
                        ? "linear-gradient(to top, var(--accent), #E8A070)"
                        : pct === 100 ? "var(--success)" : "var(--border-strong)",
                    }}
                  />
                </div>
                <span className={`text-[9px] font-bold ${isToday ? "text-[var(--accent)]" : "text-[var(--muted-2)]"}`}>
                  {DAYS_SHORT[i]}
                </span>
              </div>
            );
      ...
      <div className="mx-5 mt-4 mb-8">
        <p className="text-[12px] font-bold text-[var(--muted-2)] mb-3">جزئیات روزانه</p>
        <div className="flex flex-col gap-2">
          {[...PAST_RECORDS].reverse().map((r, i) => {
            const pct = r.tasksTotal ? Math.round((r.tasksDone / r.tasksTotal) * 100) : 0;
            const label = i === 0 ? "امروز" : i === 1 ? "دیروز" : DAYS_FA[PAST_RECORDS.length - 1 - i];
            return (
              <div key={r.date} className="bg-white rounded-2xl border border-[var(--border)] px-4 py-3 flex items-center gap-3">
                ...
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-[12px] font-bold ${
                  pct === 100 ? "bg-[#EAF5EC] text-[var(--success)]" : pct >= 50 ? "bg-[var(--accent-soft)] text-[var(--accent)]" : "bg-[#F5F5F5] text-[var(--muted-2)]"
                }`}>
                  {pct === 100 ? "✓" : `${pct}%`}
                </div>
              </div>
