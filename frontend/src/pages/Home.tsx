...
const typeBg: Record<Task["type"], string> = {
  study: "#5C8BA820", test: "#9B7AAD20", quiz: "var(--accent)20", review: "var(--success)20", practice: "#7A8AAD20",
};
const typeColor: Record<Task["type"], string> = {
  study: "#5C8BA8", test: "#9B7AAD", quiz: "var(--accent)", review: "var(--success)", practice: "#7A8AAD",
};
  ...
  return (
    <div className="flex flex-col items-center">
      <svg width="130" height="130" viewBox="0 0 130 130">
        ...
        <text x="65" y="58" textAnchor="middle" fontSize="26" fontWeight="800" fill="var(--text)" fontFamily="Vazirmatn,sans-serif">{done}</text>
        <text x="65" y="74" textAnchor="middle" fontSize="11" fill="var(--muted)" fontFamily="Vazirmatn,sans-serif">از {total} تکلیف</text>
        <text x="65" y="89" textAnchor="middle" fontSize="13" fontWeight="700" fill="var(--accent)" fontFamily="Vazirmatn,sans-serif">{pct}٪</text>
      </svg>
      <p className="text-[11px] font-bold text-[var(--muted-2)] mt-1">پیشرفت امروز</p>
    </div>
  );
}
...
function GateModal({ name, onAccept, onDismiss }: { name: string; onAccept: () => void; onDismiss: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/30 backdrop-blur-[2px] p-4">
      <div className="w-full max-w-[430px] bg-[var(--surface)] rounded-3xl p-7 shadow-2xl mb-2">
        <div className="w-10 h-10 rounded-2xl bg-[var(--accent)]/10 flex items-center justify-center mb-4">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>
          </svg>
        </div>
        <h2 className="font-display text-2xl text-[var(--text)] leading-snug mb-2">
        ...
        <button onClick={onAccept}
          className="w-full py-4 rounded-2xl bg-[var(--accent)] text-[var(--surface)] font-bold text-[14px] hover:bg-[#A85C38] active:scale-95 transition-all">
          تکمیل پروفایل
        </button>
        <button onClick={onDismiss}
          className="w-full mt-3 py-3 rounded-2xl text-[var(--muted)] text-[13px] font-semibold hover:bg-[var(--border)] transition-colors">
          بعداً
        </button>
      </div>
    </div>
...
function TaskSheet({ task, onClose, onPostpone }: { task: Task; onClose: () => void; onPostpone: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end justify-center bg-black/25 backdrop-blur-[2px]" onClick={onClose}>
      <div className="w-full max-w-[430px] bg-[var(--surface)] rounded-t-3xl p-6 pb-10 shadow-2xl"
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1">
            <p className="font-bold text-[16px] text-[var(--text)] leading-snug">{task.title}</p>
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-[11px] font-bold" style={{ color }}>{task.subject}</span>
              <span className="text-[11px] text-[var(--placeholder)]">·</span>
              <span className="text-[11px] text-[var(--muted-2)] font-medium">{typeLabels[task.type]}</span>
            </div>
          </div>
        </div>

        <p className="text-[13px] text-[var(--brown-text)] leading-relaxed mb-5 bg-white rounded-2xl px-4 py-3 border border-[var(--border)]">
          {task.description}
        </p>

        <div className="flex gap-2 mb-5">
          <div className="flex-1 bg-[var(--border)] rounded-xl px-3 py-2.5 text-center">
            <p className="text-[10px] font-bold text-[var(--muted-2)]">مدت</p>
            <p className="text-[13px] font-bold text-[var(--text-strong)] mt-0.5">{task.duration}</p>
          </div>
          <div className="flex-1 bg-[var(--border)] rounded-xl px-3 py-2.5 text-center">
            <p className="text-[10px] font-bold text-[var(--muted-2)]">ساعت</p>
            <p className="text-[13px] font-bold text-[var(--text-strong)] mt-0.5" dir="ltr">{task.scheduledTime}</p>
          </div>
        </div>

        <div className="flex gap-2.5">
          <button onClick={onPostpone}
            className="flex-1 py-3.5 rounded-2xl border border-[var(--border-strong)] text-[var(--muted)] font-bold text-[13px] hover:bg-[var(--border)] transition-colors">
            تعویق به فردا
          </button>
          <button onClick={onClose}
            className="flex-1 py-3.5 rounded-2xl bg-[var(--accent)] text-[var(--surface)] font-bold text-[13px] hover:bg-[#A85C38] transition-colors">
            بستن
          </button>
        </div>
      </div>
...
function QuizResultModal({ task, onClose, onSubmit }: { task: Task; onClose: () => void; onSubmit: (time: string, pct: string) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/30 backdrop-blur-[2px] p-6" onClick={onClose}>
      <div className="w-full max-w-[360px] bg-[var(--surface)] rounded-3xl p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="w-9 h-9 rounded-xl mb-4 flex items-center justify-center"
          style={{ background: typeBg[task.type], color: typeColor[task.type] }}>
          <TaskTypeIcon type={task.type} />
        </div>
        <h3 className="font-display text-xl text-[var(--text)] mb-1">{task.title}</h3>
        <p className="text-[12px] text-[var(--muted-2)] mb-5">نتیجه‌ی {typeLabels[task.type]} رو وارد کن</p>

        <div className="flex flex-col gap-3 mb-5">
          <div>
            <label className="text-[11px] font-bold text-[var(--muted)] mb-1.5 block">زمان صرف‌شده (دقیقه)</label>
            <input type="number" value={time} onChange={e => setTime(e.target.value)}
              placeholder="مثلاً ۳۵" dir="ltr"
              className="w-full bg-white border-2 border-[var(--border-strong)] focus:border-[var(--accent)] outline-none rounded-xl px-4 py-3 text-[14px] font-bold text-[var(--text)] placeholder:text-[var(--placeholder)] transition-colors text-left" />
          </div>
          <div>
            <label className="text-[11px] font-bold text-[var(--muted)] mb-1.5 block">درصد پاسخ درست</label>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => onSubmit(time, pct)} disabled={!time || !pct}
            className={`flex-1 py-3.5 rounded-2xl font-bold text-[13px] transition-all ${
              time && pct ? "bg-[var(--accent)] text-[var(--surface)] hover:bg-[#A85C38]" : "bg-[var(--border-strong)] text-[#B0A898] cursor-not-allowed"
            }`}>
            ثبت نتیجه
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[var(--surface)] pb-32">
      ...
      <div className="mx-4 mt-6">
        <div className="bg-white rounded-3xl p-5 shadow-sm border border-[var(--border)] flex flex-col">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2.5">
              <button onClick={() => { onInteract(); nav("profile"); }}
                className="w-9 h-9 rounded-xl bg-[var(--border)] flex items-center justify-center text-[var(--muted)] hover:bg-[var(--border-strong)] transition-colors flex-shrink-0">
              ...
              <div>
                <p className="text-[10px] text-[var(--muted-2)] font-medium">{greeting}</p>
                <p className="font-display text-[17px] text-[var(--text)] leading-tight">{userData.name}</p>
              </div>
            </div>

            {/* Left side: streak (RTL: last = leftmost) */}
            <button onClick={() => { onInteract(); nav("streak"); }}
              className="flex items-center gap-1.5 bg-[var(--accent-soft)] border border-[#FFDDCC] rounded-xl px-3 py-1.5 hover:bg-[#FFE8DC] transition-colors">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="var(--accent)">
                <path d="M12 2C9 7 6 8 7 13c.7 3 3 5 5 5s4.3-2 5-5c1-5-2-6-5-11z"/>
              </svg>
              <span className="text-[13px] font-bold text-[var(--accent)]">{STREAK}</span>
            ...
            {heroMode === "ai" && (
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="w-12 h-12 rounded-2xl bg-[var(--text)] flex items-center justify-center">
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--surface)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                ...
                <div className="relative h-2.5 bg-[var(--border)] rounded-full overflow-hidden mb-2">
                  <div className="absolute inset-y-0 right-0 rounded-full transition-all duration-1000"
                    style={{ width: `${AI_LEVEL}%`, background: "linear-gradient(to left, var(--accent), #E8A070)" }} />
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-[var(--border)] flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xl font-bold text-[var(--success)]">۴۰</p>
                  </div>
                  <div className="text-left">
                    <p className="text-[10px] text-[var(--muted-2)]">روز تا کنکور</p>
                    <p className="text-xl font-bold text-[var(--accent)]">{daysLeft}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {heroMode === "countdown" && (
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[11px] text-[var(--muted-2)] mt-1">{userData.examYear} · {userData.major}</p>
                </div>
                <div className="flex flex-col gap-2 text-left">
                  <div className="bg-[var(--accent-soft)] rounded-2xl px-4 py-3">
                    <p className="text-[9px] font-bold text-[var(--muted-2)]">تکمیل امروز</p>
                    <p className="text-[20px] font-bold text-[var(--accent)] leading-none">{done}<span className="text-[12px] text-[#C4A080]">/{tasks.length}</span></p>
                  </div>
                  <div className="bg-[var(--border)] rounded-2xl px-4 py-3">
                    <p className="text-[9px] font-bold text-[var(--muted-2)]">استریک</p>
                    <p className="text-[20px] font-bold text-[var(--text)] leading-none">{STREAK}<span className="text-[12px] text-[var(--muted-2)]"> روز</span></p>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex flex-col gap-3 text-left">
                  <div>
                    ...
                    <p className="text-[36px] font-bold text-[var(--accent)] leading-none">{daysLeft}</p>
                  </div>
                  <div className="w-24 h-px bg-[var(--border)]" />
                  <div>
                    <p className="text-[10px] font-bold text-[var(--muted-2)]">استریک</p>
                    <p className="text-[24px] font-bold text-[var(--text)] leading-none">{STREAK}<span className="text-[11px] text-[var(--muted-2)] font-medium"> روز</span></p>
                  </div>
                </div>
              </div>
            )}
      ...
      <div className="px-4 mt-5">
        <div className="flex items-center justify-between mb-3">
          <button onClick={() => setShowTime(s => !s)}
            className="text-[11px] font-bold text-[var(--muted-2)] bg-[var(--border)] px-3 py-1 rounded-full hover:bg-[var(--border-strong)] transition-colors">
            ...
            return (
              <div key={task.id}
                className={`bg-white rounded-2xl border flex items-stretch overflow-hidden transition-all ${
                  task.done ? "border-[var(--border)] opacity-55" : "border-[var(--border)] hover:border-[var(--border-strong)] shadow-sm"
                }`}
              >
                ...
                <div className="flex items-center pr-3.5">
                  <button onClick={() => handleCheckbox(task)}
                    className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all flex-shrink-0 ${
                      task.done ? "border-[var(--accent)] bg-[var(--accent)]" : isQuizOrTest ? "border-[#9B7AAD] hover:border-[#7A4A9A]" : "border-[#D5CCC3] hover:border-[var(--accent)]"
                    }`}
                  >
                    {task.done && (
                ...
                <button onClick={() => setSelectedTask(task)}
                  ...
                  <p className={`text-[13px] font-semibold leading-snug ${task.done ? "line-through text-[#B0A898]" : "text-[var(--text)]"}`}>
                    {task.title}
                  </p>
                  <p className="text-[11px] text-[var(--muted-2)] mt-0.5 font-medium line-clamp-1">{task.description}</p>
                ...
                {/* Time/Duration */}
                <div className="flex flex-col items-center justify-center px-3 py-4 min-w-[58px] bg-[#FAFAF8] border-r border-[var(--border)]">
                  <span className="text-[10px] font-bold text-[var(--muted-2)] text-center leading-tight" dir="ltr">
      ...
      <div className="px-4 mt-4 grid grid-cols-3 gap-2.5">
        {[
          { label: "ریکاوری", sub: "خواب و خلق‌وخو", screen: "recovery" as const, bg: "#EDF5F0", color: "var(--success)",
          ...
          <button key={label} onClick={() => { onInteract(); nav(screen); }}
            className="bg-white rounded-2xl border border-[var(--border)] p-3 flex flex-col items-center gap-2 hover:border-[var(--border-strong)] hover:shadow-sm active:scale-95 transition-all">
            ...
            <div className="text-center">
              ...
              <p className="text-[10px] text-[var(--muted-2)] font-medium leading-tight mt-0.5">{sub}</p>
            </div>
          </button>
        ))}
      </div>
      ...
      <div className="fixed bottom-0 inset-x-0 max-w-[430px] mx-auto px-4 pb-7 pt-6 md:hidden"
        ...
        <button onClick={() => { onInteract(); nav("chat"); }}
          className="w-full h-[52px] rounded-full bg-[var(--text)] text-[var(--surface)] font-bold text-[14px] flex items-center justify-center gap-2.5 hover:bg-[var(--text-strong)] active:scale-95 transition-all shadow-lg">
          گفتگو با بوم
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
