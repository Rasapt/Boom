...
function DesktopSidebar({ screen, nav, dark, toggleDark }: { screen: Screen; nav: (s: Screen) => void; dark: boolean; toggleDark: () => void }) {
  return (
    <aside className="hidden md:flex flex-col fixed top-0 right-0 bottom-0 w-[220px] bg-white border-r border-[var(--border)] z-30">
      {/* Logo / Name */}
            ...
            <button key={item.screen} onClick={() => nav(item.screen)}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-right transition-all ${
                active
                  ...
                  : "text-[var(--muted)] hover:bg-[var(--surface-2)] hover:text-[var(--text-strong)]"
              }`}
            >
      ...
      <div className="px-4 pb-3">
        <button onClick={toggleDark} className="w-full flex items-center justify-between px-4 py-3 rounded-2xl bg-[var(--surface-2)] text-[var(--muted)] hover:text-[var(--text)]">
          <span className="font-semibold text-[13px]">{dark ? "حالت روشن" : "حالت تاریک"}</span>
...
function Shell({ children, screen, nav, dark, toggleDark }: { children: React.ReactNode; screen: Screen; nav: (s: Screen) => void; dark: boolean; toggleDark: () => void }) {
  const hideSidebar = ["landing", "login", "signup"].includes(screen);
  return (
    <div dir="rtl" className="min-h-screen bg-[var(--page-bg)]">
      {!hideSidebar && <DesktopSidebar screen={screen} nav={nav} dark={dark} toggleDark={toggleDark} />}
...
export default function App() {
  ...
  return (
    <Shell screen={screen} nav={nav} dark={dark} toggleDark={toggleDark}>
